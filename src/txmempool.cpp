...
void CTxMemPool::addUnchecked(const CTxMemPoolEntry &entry, setEntries &setAncestors, bool validFeeEstimate)
{
    ...
    if (tx.nVersion == QUAGBA_TX_VERSION_MN_REGISTER) {
        CProRegTx proTx;
    ...
    } else if (tx.nVersion == QUAGBA_TX_VERSION_MN_UPDATE_SERVICE) {
        CProUpServTx proTx;
        if(GetTxPayload(tx, proTx)) {
            mapProTxRefs.emplace(proTx.proTxHash, tx.GetHash());
            mapProTxAddresses.emplace(proTx.addr, tx.GetHash());
        }
    } else if (tx.nVersion == QUAGBA_TX_VERSION_MN_UPDATE_REGISTRAR) {
        CProUpRegTx proTx;
        if(GetTxPayload(tx, proTx)) {
            mapProTxRefs.emplace(proTx.proTxHash, tx.GetHash());
            mapProTxBlsPubKeyHashes.emplace(proTx.pubKeyOperator.GetHash(), tx.GetHash());
    ...
    } else if (tx.nVersion == QUAGBA_TX_VERSION_MN_UPDATE_REVOKE) {
        CProUpRevTx proTx;
        if(GetTxPayload(tx, proTx)) {
            mapProTxRefs.emplace(proTx.proTxHash, tx.GetHash());
            auto dmn = deterministicMNManager->GetListAtChainTip().GetMN(proTx.proTxHash);
...
void CTxMemPool::removeUnchecked(txiter it, MemPoolRemovalReason reason)
{
    ...
    // Quagba deal with pro tx stuff first
    ...
    if (it->GetTx().nVersion == QUAGBA_TX_VERSION_MN_REGISTER) {
        CProRegTx proTx;
    ...
    } else if (it->GetTx().nVersion == QUAGBA_TX_VERSION_MN_UPDATE_SERVICE) {
        CProUpServTx proTx;
        if (GetTxPayload(it->GetTx(), proTx)) {
            eraseProTxRef(proTx.proTxHash, it->GetTx().GetHash());
            mapProTxAddresses.erase(proTx.addr);
        }
    } else if (it->GetTx().nVersion == QUAGBA_TX_VERSION_MN_UPDATE_REGISTRAR) {
        CProUpRegTx proTx;
        if (GetTxPayload(it->GetTx(), proTx)) { 
            eraseProTxRef(proTx.proTxHash, it->GetTx().GetHash());
            mapProTxBlsPubKeyHashes.erase(proTx.pubKeyOperator.GetHash());
        }
    } else if (it->GetTx().nVersion == QUAGBA_TX_VERSION_MN_UPDATE_REVOKE) {
        CProUpRevTx proTx;
        if (GetTxPayload(it->GetTx(), proTx)) {
            eraseProTxRef(proTx.proTxHash, it->GetTx().GetHash());
        }
...
void CTxMemPool::removeConflicts(const CTransaction &tx)
{
    ...
    for (const CTxIn &txin : tx.vin) {
        ...
        if (it != mapNextTx.end()) {
            ...
            if (txConflict != tx)
            {
                if (txConflict.nVersion == QUAGBA_TX_VERSION_MN_REGISTER) {
